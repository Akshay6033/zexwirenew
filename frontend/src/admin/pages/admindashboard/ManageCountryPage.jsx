...
const INITIAL_FORM = { id: null, country_name: "" };
...
function ManageCountryPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await adminDashboardApi.getCountries(query);
      ...
      setTotal(0);
      setFilteredTotal(0);
      toast.error(error.response?.data?.message || "Could not load countries");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (row) => {
    setForm({ id: row.id, country_name: row.country_name || "" });
    setFormError("");
    setModalOpen(true);
  };

  const onCountryNameChange = (value) => {
    const cleaned = String(value || "").replace(/[^A-Za-z\s]/g, "");
    setForm((prev) => ({ ...prev, country_name: cleaned }));
    setFormError("");
  };

  const onSubmit = async () => {
    const countryName = String(form.country_name || "").trim();
    if (!countryName) {
      setFormError("Enter country name");
      return;
    }
    if (!/^[A-Za-z\s]+$/.test(countryName)) {
      setFormError("Country name must contain only letters and spaces");
      return;
    }
    setSubmitting(true);
    try {
      if (form.id) {
        await adminDashboardApi.updateCountry(form.id, { country_name: countryName });
        toast.success("Country updated successfully");
      } else {
        await adminDashboardApi.createCountry({ country_name: countryName });
      ...
      const message = error.response?.data?.message || "Could not save country";
  ...
  const toggleStatus = async (row) => {
    try {
      await adminDashboardApi.updateCountryStatus(row.id, { active: row.active ? 0 : 1 });
      toast.success(row.active ? "Country deactivated successfully" : "Country activated successfully");
      load();
    } catch (error) {
      toast.error(error.response?.data?.message || "Could not update country status");
    }
  };

  const deletePermanently = async (row) => {
    const ok = await confirm("Are you sure delete permanently Country ?");
    if (!ok) return;
    try {
      await adminDashboardApi.deleteCountryPermanently(row.id);
      toast.success("Country deleted permanently");
      load();
    } catch (error) {
      toast.error(error.response?.data?.message || "Could not delete country");
    }
  };
  ...
  return (
    <div className="container-fluid manage-country-page">
      <div className="page-head manage-user1 d-flex justify-content-between align-items-center">
        <h4 className="mt-2 mb-2">Manage Country</h4>
        <div className="d-flex gap-2">
          ...
          <button type="button" className="btn add-vendor-btn" onClick={openAddModal}>Add New Country</button>
        </div>
      </div>
            ...
            <table className="table table-bordered table-bg1 mb-0">
              <thead>
                <tr>
                  ...
                  <th><button type="button" className="login-log-sort" onClick={() => onSort("country_name")}>Country Name <span>{sortArrow("country_name")}</span></button></th>
                  <th>Edit Country</th>
              ...
              <tbody>
                {!loading && rows.length === 0 && (
                  <tr><td colSpan={4} className="text-center text-muted py-4">No countries found.</td></tr>
                )}
                {rows.map((row, index) => (
                  <tr key={row.id}>
                    ...
                    <td>
                      <div className="d-flex gap-2">
                        <button type="button" className="edit-icon-btn" onClick={() => openEditModal(row)} aria-label="Edit country">✎</button>
                        <button type="button" className="edit-icon-btn country-delete-icon-btn" onClick={() => deletePermanently(row)} aria-label="Delete country permanently">🗑</button>
                      </div>
                    </td>
                    <td>
                      <button
                        ...
                        aria-label={row.active ? "Deactivate country" : "Activate country"}
                      >
        ...
        <div className="modal-backdrop-lite">
          <div className="modal-card-lite distribution-modal">
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h5 className="mb-0">{form.id ? "Edit Country" : "Add New Country"}</h5>
              <button type="button" className="btn-close" onClick={() => setModalOpen(false)} />
            </div>
            <div className="mb-3">
              <label className="form-label">Country Name</label>
              <input className="form-control" value={form.country_name} onChange={(e) => onCountryNameChange(e.target.value)} />
              {formError ? <div className="field-error mt-1">{formError}</div> : null}
            </div>
            <div className="d-flex justify-content-end gap-2">
              <button type="button" className="btn btn-secondary" onClick={() => setModalOpen(false)}>Cancel</button>
              <button type="button" className="btn btn-primary" onClick={onSubmit} disabled={submitting}>{submitting ? "Please wait..." : form.id ? "Update Country" : "Submit"}</button>
            </div>
          </div>
        </div>
      )}
